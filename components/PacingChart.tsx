...
// Small hand-rolled SVG bar chart. No dependency, no library bloat — the
// visual is simple enough (bars + two reference lines) that hand-rolled
// gives more control and lighter output than Recharts/Chart.js.
//
// Contract:
//   buckets: pre-computed by lib/goals.ts::bucketLogsForChart
//   goal / benchmark: horizontal reference values; 0 hides the line
//   color: tailwind background-color utility for the bars
...
interface Props {
  ...
  goal: number;
  benchmark: number;
  barColorClass: string;      // e.g. "fill-blue-500"
  currentBarColorClass?: string; // e.g. "fill-blue-700"
  goalStrokeClass?: string;   // e.g. "stroke-blue-700"
  benchmarkStrokeClass?: string;
  emptyLabel?: string;
}

const CHART_HEIGHT = 160; // px of drawing area (excluding X-axis labels)
...
export function PacingChart({
  ...
  // If everything is 0, use a sentinel so we don't divide by zero and the
  // baseline still renders.
  const yMax = rawMax > 0 ? Math.ceil(rawMax * 1.15) : 1;

  const bucketCount = Math.max(1, buckets.length);
  // Compute an inner drawing width via a viewBox scale — we render at 100%
  // container width so the whole chart is responsive. viewBox is a fixed
  // logical space and the browser scales it.
  const totalBarSpace = 400; // logical viewBox units for bars
  const barWidth =
    (totalBarSpace - BAR_GAP * (bucketCount - 1)) / bucketCount;
          ...
          >
            ...
            {buckets.map((b, i) => {
              ...
              return (
                ...
                >
                  {/* Full-height hit target so hover works even for 0-count bars */}
                  <rect
                    x={x}
                    y={TOP_PAD}
                    width={barWidth}
...
// The goal line legend uses a solid rect matching the stroke color.
// Translate a stroke-* class to a bg-* class for the swatch. Tailwind's
// palette is consistent so this is safe.
